...
| Shared preview Worker | `jinzheio-preview` |
| PR preview cleanup | `pnpm exec wrangler delete jinzheio-pr-<number> --force` |
...
`wrangler.toml` 显式设置 `preview_urls = true`。PR preview workflow 会从 `dist/server/wrangler.json` 生成无生产 route、无 session KV 的 `dist/server/preview.wrangler.json`，部署成 `jinzheio-pr-<number>` preview Worker。它还会部署共用 Worker `jinzheio-preview`，用于 OAuth callback 等需要稳定 URL 的测试。PR 关闭或合并后，workflow 只删除 `jinzheio-pr-<number>`，不删除 `jinzheio-preview`。
